...
const DistributeurDetailsPage = () => {
  ...
  const { data: contacts } = useQuery({
    queryKey: ['contacts', 'distributeur', id],
  ...
  if (distributeur.type !== 'DISTRIBUTEUR') {
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <h1 className="text-3xl font-bold mt-4">{distributeur.name}</h1>
        </div>
        <Link to={`/distributeurs/${id}/edit`}>
          <Button>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div>
              ...
              <p className="font-semibold">{distributeur.name}</p>
            </div>
            <div>
            ...
            {distributeur.groupe && (
              <div>
                ...
                <p className="font-semibold">{distributeur.groupe.name}</p>
              </div>
            )}
            <div>
            ...
            {distributeur.address && (
              <div>
                ...
                <p className="font-semibold">{distributeur.address}</p>
              </div>
            )}
            {(distributeur.city || distributeur.postal_code) && (
              <div>
            ...
            {distributeur.country && (
              <div>
                ...
                <p className="font-semibold">{distributeur.country}</p>
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-3">
            {distributeur.email && (
              <div>
                ...
                <p className="font-semibold">{distributeur.email}</p>
              </div>
            )}
            {distributeur.phone && (
              <div>
                <p className="text-sm text-muted-foreground">Téléphone</p>
                <p className="font-semibold">{distributeur.phone}</p>
              </div>
            )}
            <div>
              <p className="text-sm text-muted-foreground">Date de création</p>
              <p className="font-semibold">{formatDate(distributeur.created_at)}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Dernière mise à jour</p>
              <p className="font-semibold">{formatDate(distributeur.updated_at)}</p>
            </div>
            {distributeur.notes && (
              <div>
                <p className="text-sm text-muted-foreground">Notes</p>
                <p className="text-sm">{distributeur.notes}</p>
              </div>
            )}
          </CardContent>
        </Card>
...
export default DistributeurDetailsPage;
