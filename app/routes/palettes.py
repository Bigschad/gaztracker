...
@router.get(
    "/next-code",
    response_model=str,
    summary="Get next palette reference code",
    description="Get the next available palette reference code (autogenerated)."
)
async def get_next_palette_code(
    db: Session = Depends(get_sync_db),
    current_user: User = Depends(get_current_user_sync),
):
    """Get the next available palette reference code."""
    try:
        return PaletteService._generate_reference_code(db)
    except Exception as e:
        logger.error(f"Error generating palette code: {str(e)}")
        raise HTTPException(
